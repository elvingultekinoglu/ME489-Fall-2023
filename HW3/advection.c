...
# include <time.h>
# include "advection.h"
#include <omp.h>//for paralel programing. 
...
int main ( int argc, char *argv[] ){
  ...
  if(argc!=4){
    printf("Usage: ./HW3 input.dat core_number wanted_analysis_number \n");
    printf("wanted_analysis_number: \n 1 = Just parallel rqsh function\n 2 = Infinity norm calculation with critical\n 3 = Infinity norm calculation with reduction\n 4 = Infinity norm calculation with atomic\n ");
    printf("Example:  ./HW3 input.dat 4 1\n");
    return -1;  
  }
  static int frame=0;
  ...
  int analysis_set=strtol(argv[3], NULL, 5);
  ...
  {
    char fname[BUFSIZ];
    sprintf(fname, "Paralel_test_%04d.csv", frame++);
  ...
  for(int step = 0; step<Nsteps; step++){
    ...
    printf("Elapsed time of parallel RhsQ solver: %f seconds\n", elapsed_time);
  ...
  for(int step = 0; step<Nsteps; step++){
    ...
    if(step%Noutput == 0){
      char fname[BUFSIZ];
      sprintf(fname, "C_Paralel_test_%04d.csv", frame++);
      solverPlot(fname, &advc.msh, advc.q);
      //printf("Scaled time : %d/%d\n", (step/1000)+1,Nsteps/1000);
          // Print the updated inf array
    ...
    // Calculate and print the elapsed time
    double elapsed_time = end_time - start_time;
    printf("Elapsed time for calculation infinity norm with critical: %f seconds\n", elapsed_time);
    printf("The infinitf norm is: %f\n",inf);

}
else if(analysis_set==3){
  //for infinity norm
  double inf = -1000;
  ...
  for(int step = 0; step<Nsteps; step++){
    ...
    if(step%Noutput == 0){
      char fname[BUFSIZ];
      sprintf(fname, "R_Paralel_test_%04d.csv", frame++);
      solverPlot(fname, &advc.msh, advc.q);
      //printf("Scaled time : %d/%d\n", (step/1000)+1,Nsteps/1000);
          // Print the updated inf array
    ...
    // Calculate and print the elapsed time
    double elapsed_time = end_time - start_time;
    printf("Elapsed time for calculation infinity norm with reduction: %f seconds\n", elapsed_time);
    printf("The infinitf norm is: %f\n",inf);
}
else if (analysis_set==4){
    double inf = -1000;
  ...
  for(int step = 0; step<Nsteps; step++){
    ...
    if(step%Noutput == 0){
      char fname[BUFSIZ];
      sprintf(fname, "A_Paralel_test_%04d.csv", frame++);
      solverPlot(fname, &advc.msh, advc.q);
      //printf("Scaled time : %d/%d\n", (step/1000)+1,Nsteps/1000);
          // Print the updated inf array
    ...
    // Calculate and print the elapsed time
    double elapsed_time = end_time - start_time;
    printf("Elapsed time for calculation infinity norm with atomic: %f seconds\n", elapsed_time);
    printf("The infinitf norm is: %f\n",inf); 
}
...
//printf("my core number: %d",my_rank);
int thread_count = omp_get_num_threads();
//printf("my total core number: %d \n",thread_count);
